...
export default function Chatbot({ onClose }) {
  const STORAGE_KEY = "chatbot_history_candidate";
  ...
  const sendMessage = async () => {
    ...
    try {
      const data = await apiFetch("/chatbot/ask", {
        ...
        body: JSON.stringify({ message: input, role: "candidate", history: nextMessages }),
      });
      setMessages((msgs) => [
        ...msgs,
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 max-w-full">
      <div className="panel flex flex-col h-[26rem] overflow-hidden relative">
        <div className="absolute inset-0 pointer-events-none bg-gradient-to-br from-blue-50/60 via-transparent to-sky-50/60" />
        <div className="px-4 py-3 border-b border-slate-200/70 bg-gradient-to-r from-blue-600 to-sky-500 text-white font-semibold flex items-center justify-between relative">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold">
              AI
            </div>
            <div className="leading-tight">
              <div className="text-sm font-semibold">Candidate Assistant</div>
          ...
          {messages.map((msg, i) => (
            <div
              ...
              className={`text-sm px-3 py-2 rounded-2xl max-w-[85%] shadow-sm ${
                msg.sender === "bot"
                  ? "bg-blue-50 text-slate-800 self-start border border-blue-100"
                  : "bg-sky-50 text-slate-800 self-end border border-sky-100"
              }`}
            >
              {msg.text}
            </div>
        ...
        >
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-gradient-to-r from-blue-600 to-sky-500 text-white rounded-br-xl disabled:bg-gray-300 text-sm font-semibold"
