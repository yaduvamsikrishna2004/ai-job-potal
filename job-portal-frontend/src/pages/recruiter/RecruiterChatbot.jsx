  ...
  const sendMessage = async () => {
    ...
    try {
      const data = await apiFetch("/chatbot/ask", {
        ...
        body: JSON.stringify({ message: input, role: "recruiter" }),
      });
      setMessages((msgs) => [
        ...msgs,
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 max-w-full">
      <div className="bg-white rounded-xl shadow-xl border flex flex-col h-96">
        <div className="p-3 border-b font-bold text-purple-700 flex items-center">
          <span className="mr-2">🤖</span> Recruiter Chatbot
        </div>
        ...
        >
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-purple-600 text-white rounded-br-xl disabled:bg-gray-300"
